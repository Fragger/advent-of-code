...
def calibrate(test, value, nums, p2flag=False):
    if not nums or value > test:
        return value == test, p2flag

    for op in (add, mul, concat):
        result = calibrate(test, op(value, nums[0]), nums[1:], (p2flag or op == concat))
        if result[0]:
            return result
...
with open(sys.argv[1] if len(sys.argv) > 1 else '../input/07') as f:
    for line in f:
        ...
        result, p2flag = calibrate(test, nums[0], nums[1:])
